...
class Artifact:
    def __init__(self, file, bid=None, polyswarmd_host=HOST, api_key=API_KEY, account=ACCOUNT):
        # file object
        self.file = file
        self.uri = ''
        self.bid = bid
        ...
        self.account_d = {'account': self.account}
    ...
    def postArtifact(self):
        ...
        try:
            response = requests.post(url, headers=headers, params=self.account_d, files=file)
        except:
    ...
    def postBounty(self, duration, keyfile=KEYFILE, password='', account=ACCOUNT):
        ...
        try:
            response = requests.post(url, headers=headers, json=data, params=self.account_d)
        except:
        ...
        for tx in transactions:
            ...
            signed.append(raw)
        d = {'transactions': signed}

        r = requests.post('http://' + self.polyswarmd_host + '/transactions', headers=headers,
                          json=d, params=self.account_d)
